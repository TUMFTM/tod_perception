...
static tod_video::VideoConfig _repliedConfig;
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "OperatorVideoConfigSend");
    ros::NodeHandle n;
    ...
    dynamic_reconfigure::Server<tod_video::VideoConfig> server;
...
void callback_status_msg(const tod_msgs::Status &msg) {
    ...
    if (connected && !_connected) {
        ...
        _mqttClient->subscribe(tod_network::MqttTopics::ActualVideoConfig, 1,
...
void callback_video_reconfigure_request(tod_video::VideoConfig &config, uint32_t level) {
    ...
    if (_receivedActualConfigFromVehicle) {
        ...
        if (_receivedActualConfigFromVehicle) {
            ...
            ROS_ERROR("%s: did not get actual video config for %s",
...
void send_desired_config_to_vehicle(const tod_video::VideoConfig &config) {
    ...
    _mqttClient->publish(tod_network::MqttTopics::DesiredVideoConfig, 1,
                         (char*) serCfg.message_start, serCfg.num_bytes);
    ROS_DEBUG("%s: Sent desired video config for camera %s on mqtt topic %s!", _nodeName.c_str(),
              config.camera_name.c_str(), tod_network::MqttTopics::DesiredVideoConfig.c_str());
}
